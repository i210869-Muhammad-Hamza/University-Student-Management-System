    ...
    protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string selectCommand = "select u.NAME, s.ROLLNO from User_ u join Student s on s.USERID= u.USERID join registered_Courses r on r.Studentid = s.ROLLNO join course c on r.courseID = c.course_ID where Cname= @course "; 
        ...
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            ...
            string evaluation = Request.QueryString["evaluation"];
            connection.Open();
            foreach (GridViewRow row in GridView1.Rows)
            {
                TextBox txtObtainedMarks = (TextBox)row.FindControl("TextBox1");
                string studentID = row.Cells[1].Text;
                string obtainedMarksStr = txtObtainedMarks.Text.Trim();
                int obtainedMarks = 0;

                if (int.TryParse(obtainedMarksStr, out obtainedMarks))
                {
                    ...
                    SqlCommand insertCommand = new SqlCommand("INSERT INTO " + evaluation + "  VALUES ( @studentID, @obtainedMarks, @courseID)", connection);
                    insertCommand.Parameters.AddWithValue("@courseid", courseID);
                    insertCommand.Parameters.AddWithValue("@studentid", studentID);
                    insertCommand.Parameters.AddWithValue("@obtainedMarks", SqlDbType.Int).Value = obtainedMarks;
        ...
        AppendToTextFile("TEACHER ENTERED MARKS SUCCESSFULLY ;");
    }
